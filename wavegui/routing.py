...
_event_handlers = {}  # dictionary of event_source => [(event_type, handler)]
_arg_handlers = {}  # dictionary of arg_name => [(handler)]
_path_handlers = []
...
def _add_handler(arg: str, func: Callable):
    if arg not in _arg_handlers:
        _arg_handlers[arg] = handlers = []
    else:
        handlers = _arg_handlers[arg]
    handlers.append((func, _get_arity(func)))


def _add_event_handler(source: str, event: str, func: Callable):
    if source not in _event_handlers:
        _event_handlers[source] = handlers = []
    else:
        handlers = _event_handlers[source]
    handlers.append((event, func, _get_arity(func)))
    ...
    def wrap(func):
        ...
        if isinstance(arg, str) and len(arg):
            if arg.startswith('#'):  # location hash
                ...
                _add_event_handler(source, event, func)
            else:
                _add_handler(arg, func)
        else:
            _add_handler(func_name, func)
...
async def _match_predicate(func: Callable, arity: int, q: Q, arg: any) -> bool:
    if arg:
    ...
    for event_source in event_sources:
        ...
        if entries:
            for entry in entries:
                event_type, func, arity = entry
                if event_type in event:
                    arg_value = event[event_type]
                    if await _match_predicate(func, arity, q, arg_value):
                        return True
